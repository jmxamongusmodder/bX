...
class PlayState extends MusicBeatState
{
	...
	//Way to disable and enable henry teleporter
	public var ovwe:Bool = true;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'plantroom': // pink stage
				...
				#if (flixel_addons < "3.0.0") cloud1 = new FlxBackdrop(Paths.image('mira/cloud 1', 'impostor'), 1, 1, true, true); #else cloud1 = new FlxBackdrop(Paths.image('mira/cloud 1', 'impostor'), XY, 0, 0); #end
				...
				#if (flixel_addons < "3.0.0") cloud2 = new FlxBackdrop(Paths.image('mira/cloud 2', 'impostor'), 1, 1, true, true); #else cloud2 = new FlxBackdrop(Paths.image('mira/cloud 2', 'impostor'), XY, 0, 0); #end
				...
				#if (flixel_addons < "3.0.0") cloud3 = new FlxBackdrop(Paths.image('mira/cloud 3', 'impostor'), 1, 1, true, true); #else cloud3 = new FlxBackdrop(Paths.image('mira/cloud 3', 'impostor'), XY, 0, 0); #end
				...
				#if (flixel_addons < "3.0.0") cloud4 = new FlxBackdrop(Paths.image('mira/cloud 4', 'impostor'), 1, 1, true, true); #else cloud4 = new FlxBackdrop(Paths.image('mira/cloud 4', 'impostor'), XY, 0, 0); #end
				...
				#else 
				cloudbig = new FlxBackdrop(Paths.image('mira/bigcloud', 'impostor'), XY, 0, 0); #end
			...
			case 'pretender': // pink stage
				...
				trace("CLOUD SPRITES SOON I JUST AAAAAAAAAA");
	...
	trace("MORE SOON?????!?!?!?");
			...
			case 'airshipRoom': // thanks fabs
				...
				var yellowdead:FlxSprite = new FlxSprite(-240, 736).loadGraphic(Paths.image('airship/newAirship/YELLOWHASDIER', 'impostor'));
				yellowdead.antialiasing = true;
				yellowdead.scrollFactor.set(1, 1);
				yellowdead.active = false;
                                yellowdead.visible = false;
				trace(SONG.song.toLowerCase());
				if (SONG.song.toLowerCase() == 'oversight')
				{
					trace('lol i made it so that yellow is dead forever');
                                        yellowdead.visible = true;
					add(yellowdead);
				}
				...
				if (isStoryMode && SONG.song.toLowerCase() == 'mando' && SONG.song.toLowerCase() == 'dlow')
				{
					...
					add(henryTeleporter);
                                        ...
                                        if(ovwe == true && FlxG.mouse.pressed)
                                        {
					     henryTeleport();
                                        }
				}
			...
			case 'idk':
				...
				var cloud:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('overlay'));
			...
			case 'idkpixely':
				...
				GameOverSubstate.characterName = 'bf-idk-deadd';

				var sky:FlxSprite = new FlxSprite(0,0).loadGraphic(Paths.image('tobypixels'));
				sky.antialiasing = false;
				sky.scrollFactor.set(1, 1);
				sky.active = false;
				add(sky);

				var cloud:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('tobylay'));
				cloud.antialiasing = false;
				cloud.scrollFactor.set(1, 1);
				cloud.active = false;
				cloud.cameras = [camOther]; // whar
				add(cloud);
			...
			case 'reactor2':
				...
				tooguswhitee = new FlxSprite(1200, 100);
				tooguswhitee.frames = Paths.getSparrowAtlas('reactor/ball lol', 'impostor');
				tooguswhitee.animation.addByPrefix('bop', 'core instance 1', 24, false);
				tooguswhitee.animation.play('bop');
				tooguswhitee.scrollFactor.set(1, 1);
				tooguswhitee.active = true;
				tooguswhitee.antialiasing = true;
				add(tooguswhitee);
...
//            sussusPenisLOL.screenCenter(X);
	...
	function henryTeleport()
	{
		...
		new FlxTimer().start(2.62, function(tmr:FlxTimer)
		{
			colorShader.amount = 1;
			FlxTween.tween(boyfriend, {"scale.y": 2.1}, 0.6, {ease: FlxEase.expoOut});
			FlxTween.tween(boyfriend, {"scale.x": 2.7}, 0.6, {ease: FlxEase.expoOut});
		});
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.SEVEN && !endingSong && !inCutscene)
		{
			...
			#if desktop
			DiscordClient.changePresence("Chart Editor of vs impostor", null, null, true);
			#end
		}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String)
	{
		if (stopEvents == false)
		{
			switch (eventName)
			{
				...
				case 'when de pixel is sus':
					iconP2.changeIcon('idkpiz');
					iconP1.changeIcon('idkbfpiz');
                                        curStage = "idkpixely";
                                        //triggerEventNote('Change Character', '1', 'black');
                                        //triggerEventNote('Change Character', '0', 'black');
				case 'revort':
					iconP2.changeIcon('idk');
					iconP1.changeIcon('idkbf');
                                        curStage = "idk";
                                        //triggerEventNote('Change Character', '1', 'black');
                                        //triggerEventNote('Change Character', '0', 'black');
				case 'banana event even tho it aint':
					henryTeleport();
				case 'Hey!':
					var value:Int = 2;
		...
		switch (curStage)
		{
			...
			case 'reactor2':
				if (curBeat % 4 == 0)
				{
					...
					tooguswhitee.animation.play('bop', true);
				}
			case 'plantroom':
