...
class AmongStoryMenuState extends MusicBeatState
{
	...
	/////////////////trace("can the cam move " + camMove + " How cool of a thing! I am definitly not adding this for crashes");
	var unlockedWeek:Array<Bool> = [true, false, false, false, true, false, false, true, false, false, false, false]; //weeks in order of files in preload/weeks
        // so the first true is red, 2nd true is maroon, and 3rd true is jorsawsee (and everything else is green yellow black pink gray tomongus henry loggo and alpha wait why is alpha a week)
	...
	override function create()
	{
		...
		#if (flixel_addons < "3.0.0")
        starsBG = new FlxBackdrop(Paths.image('menuBooba/starBG', 'impostor'), 0, 1, false, true);
		#else
		...
		#if (flixel_addons < "3.0.0")
		starsFG = new FlxBackdrop(Paths.image('menuBooba/starFG', 'impostor'), 0, 1, false, true);
		#else
		starsFG = new FlxBackdrop(Paths.image('menuBooba/starFG', 'impostor'), XY, 0, 0); //        starsFG = new FlxBackdrop(Paths.image('menuBooba/starFG', 'impostor'), 0, 1, false, true);
		#end
		...
		DiscordClient.changePresence("STORI SHITTER", null);
		#end
		...
		for (i in 0...12)
		{
			...
			//trace("ok");
		//	FlxMouseEventManager.add(weekCircle, onMouseDown());
          //  public function onMouseDown(weekCircle:FlxSprite) { changeWeek(); trace("doing stuffie"); FlxG.sound.play(Paths.sound('scrollMenu')); }
            if ((curWeek - 1 == i && unlockedWeek[curWeek - 1] || curWeek + 1 == i && unlockedWeek[curWeek + 1])){
