...
class AmongFreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		#else
		starsBG = new FlxBackdrop(Paths.image('menuBooba/starBG', 'impostor'), XY, 0, 0);
		#end
		...
		#else
		starsFG = new FlxBackdrop(Paths.image('menuBooba/starFG', 'impostor'), XY, 0, 0);
		#end
		...
		portrait.animation.addByIndices('bana', 'Character', [25], null, 24, true);
		...
		portra = new FlxSprite();
		portra.frames = Paths.getSparrowAtlas('freeplay/ports', 'impostor');
		portra.animation.addByIndices('bana', 'Character', [0], null, 24, true);
		portra.animation.play('bana');
		portra.antialiasing = true;
                portra.visible = false;
		portra.setPosition(304.65, -100);
		portra.updateHitbox();
		portra.scrollFactor.set();
		add(portra);
	...
	public static function addWeeks():Array<FreeplayWeek>
	{
		...
		weeks.push({
			songs: [
				["O2", "jorsawsee", 'jorsawsee', FlxColor.fromRGB(38, 127, 230), BEANS, [], 250, false],
				["Voting Time", "votingtime", 'warchief', FlxColor.fromRGB(153, 67, 196), BEANS, [], 300, false],
				["Turbulence", "redmungus", 'redmunp', FlxColor.RED, BEANS, [], 350, false],
				["Victory", "warchief", 'warchief', FlxColor.fromRGB(153, 67, 196), BEANS, [], 415, false]
			],
		...
		weeks.push({
			songs: [
				...
				["Who", "kills", 'kills', FlxColor.fromRGB(22, 65, 240), BEANS, [], 500, false],
		...
		weeks.push({
			songs: [
				...
				["Annoying", "idk", 'nuzzus', FlxColor.fromRGB(211, 162, 124), SPECIAL, ['Ow', 'Idk'], 0, false]
			],
		...
		weeks.push({ // THE STANELY PARABLE ADVENTURE LINE SONG SECTION AND WHATEVER I FEEL LIKE FITS.
			songs: [
				["Posussium", "cvp", 'chips', FlxColor.fromRGB(255, 60, 38), BEANS, [], 250, false],
				["Top 25", "top", 'top', FlxColor.fromRGB(188, 106, 223), SPECIAL, ['posussium'], 0, false]
			],
